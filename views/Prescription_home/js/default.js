...
$.ajax({
type:'post',
url:root_url+"Prescription_home/load_prescriptions",
dataType:'html',
...
$('.patient_age').val(data.val_3+' Years');
calage(data.val_4);
$('.patient_weight').val(data.val_2+' kg');
}
});
return false;
});
...
form.subscribe('parsley:form:success', function (e) {
	var data=$('.form_main').serialize();
			var root_url=$("#root_url").val();
			var url=root_url+"Prescription_home/save_main";
	$.ajax({
	type:'post',
	url:url,
	data:data,
	...
	success:function(data){
		    if(data.reply=="10"){
		    	invoice_id=data.id;
				var root_url=$("#root_url").val();
				var url=root_url+"Prescription_home/save_sub";
				var jsonObj = {};
...
function loaddata_note(page,id)
{
var root_url=$("#root_url").val();
var url=root_url+"Prescription_home/load_note";
$.ajax({
  type:'post',
  url:url,
  data :{page:page,pid:id},
...
function deselect2(e) {
  $('.pop2').slideFadeToggle2(function() {
    e.removeClass('selected');
  });    
}

$(function() {
  $('.pt_nots').on('click', function() {
  	$id=$('.patient_number').val();
  	$nme=$("#customerselect option:selected").text();
    ...
    if($(this).hasClass('selected')) {
      ...
      $(this).addClass('selected');
      $('.pop2').slideFadeToggle2();
    }
    return false;
  });

  $('.close_n').on('click', function() {
    deselect2($('#contact'));
    return false;
  });
});

$.fn.slideFadeToggle2 = function(easing, callback) {
  return this.animate({ opacity: 'toggle', height: 'toggle' }, 'fast', easing, callback);
};
