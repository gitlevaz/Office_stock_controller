...
form.subscribe('parsley:form:success', function (e) {
	var data=$('.form_main').serialize();
			...
			var url=root_url+"Substore_issuenote/save_main";
	$.ajax({
	type:'post',
	url:url,
	data:data,
	...
	success:function(data){
		    if(data.reply=="10"){
		    	invoice_id=data.id;
				var root_url=$("#root_url").val();
				var url=root_url+"Substore_issuenote/save_sub";
				var jsonObj = {};
				...
				$.ajax({
				  ...
				  success:function(data){
				     if(data.reply=="10"){
				      $("#msg_box").html('');  
				      $('#total_val').text('0.00');
				      $("#msg_box").append("<div class='alert alert-success'>Issue note Saved Successfully.</div>");
				      $('.form_main')[0].reset();
				      $('.grn_rows').remove();
				      getdata_index('a b issuenotemaster');
				      ...
				      if(invoice_id != null || invoice_id !=''){
				      	var urlsub=root_url+"Substore_issuenote/remove_issuenote";
				      	$.ajax({
				      		type:'post',
				      		url:urlsub,
				      		data:{id:invoice_id},
...
$("#stockitems").change(function(){
		var val=$(this).val();
		var root_url=$("#root_url").val();
		var url=root_url+"Substore_issuenote/load_item";
	$.ajax({
		type:'post',
		url:url,
		data:{id:val},
