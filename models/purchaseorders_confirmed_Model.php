...
<?php
class purchaseorders_confirmed_Model extends model
{
	...
	function load(){
		...
		$res=R::getAll("SELECT * FROM ordermaster WHERE status='1' ORDER BY id DESC LIMIT $start,$per_page");	
		if($res){
		...
		echo "  <ul class='data_list' id='ul_".$value['id']."'>
				...
				echo "	<li><button type='button' class='btn btn-warning btn_unconfirm' id='d_".$value['id']."'>Unconfirm Order</button></li>";

		echo "</ul>";
		echo "<button type='button' class='btn btn-primary btn_edit2' id='e_".$value['id']."'>Show Order</button>
	...
	echo"	<td><input type='text' value='".number_format($val['amount'], 2, '.', '')."' id='amount_".$val['id']."' 
		...
		$query_pag_num=R::getAll("SELECT COUNT(*) FROM ordermaster WHERE status='1'");	
		if($query_pag_num){
...
function unconfirmorder(){
	$id=$_POST['id'];
    $rest=R::exec("UPDATE ordersub SET status='0' WHERE order_no='$id' ");
    $res=R::exec("UPDATE ordermaster SET status='0' WHERE id='$id' ");
     if($res=='1'){
       	 $data=array("reply"=>"10");
	    }else{
	     $data=array("reply"=>"20");    
