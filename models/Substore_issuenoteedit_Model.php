...
<?php
class Substore_issuenoteedit_Model extends model
{
	...
	function load(){
		...
		$res=R::getAll("SELECT * FROM issuenotemaster ORDER BY id DESC LIMIT $start,$per_page");	
		if($res){
		...
		$isn_id=$value['id'];
		$result=R::getAll("SELECT * FROM issuenotesub WHERE isn_no=$isn_id");
		echo " 	
		...
		echo "<tr class='datatr_".$val['id']."'>
				...
				<td>
	        	<select id='stockitems' class='sdata_val2_".$val['id']." form-control' data-id-aa='".$val['id']."' data-id-bb='".$val['isn_no']."' name='val_2' required=''>";
	    ...
	    echo"   </select>
				...
				<td>
					<input type='number' value='".$val['unit_price']."' style='width:100px' class='unitprice data_".$val['id']." sdata_val3_".$val['id']." form-control' data-id-aa='".$val['id']."' data-id-bb='".$val['isn_no']."' numberonly required='' min='1'>
				</td>
				<td>
				    <input type='number' value='".$val['quantity']."' style='width:100px' class='quantity sdata_val4_".$val['id']." form-control' id='".$val['id']."' data-id-bb='".$val['isn_no']."' numberonly required='' min='1'>
				</td>
	...
	echo"	<td><input type='text' value='".number_format($val['amount'], 2, '.', '')."' id='amount_".$val['id']."' 
			class='totamount_".$val['isn_no']." sdata_val8_".$val['id']." form-control'></td>
				<td><button type='button' class='btn btn-success btn_subedit' id='e_".$val['id']."' data-id-bb='".$val['isn_no']."'>Update</button></td>
				<td><button type='button' class='btn btn-danger btn_subdelete' id='d_".$val['id']."' data-id-bb='".$val['isn_no']."'>Delete</button><td>
				</tr>";
		}
		echo "
		<tr><td></td> <td></td> <td></td> <td></td>
		<td>Total</td><td><span class='sub_total_".$val['isn_no']."'></span></td>
		<td></td> <td></td>
		</tr>
		...
		$query_pag_num=R::getAll("SELECT COUNT(*) FROM issuenotemaster");	
		if($query_pag_num){
...
function delete_sub(){
 $data=null;
 $id=$_POST['id'];
 $res=parent::delete2("issuenotesub",$id);
 $data=array("reply"=>"10");
...
$val_10=$_POST['val_10'];
  ...
  $rest=R::exec("UPDATE issuenotesub SET item_code='$val_3',item_name='$val_4',unit_price='$val_5',quantity='$val_6',unit='$val_7',amount='$val_10' WHERE id='$val_1' AND isn_no='$val_2' ");
  $rest=R::exec("UPDATE stockitems SET quantity=quantity-'$val_6' WHERE id='$val_3'");
     if($rest=='1'){
     $data=array("reply"=>"10");
     }else{
     $data=array("reply"=>"20");    
...
function create_new(){
		...
		echo"
				...
				<td><button type='button' class='btn btn-info new_save' id='newsave_".$_POST['count']."' data-id-isnno='".$_POST['isnno']."'>Save</button></td>
...
$res=parent::insert("issuenotesub",array("isn_no"=>$isnno,"item_code"=>$val_1,"item_name"=>$val_2,"unit_price"=>$val_3,"quantity"=>$val_4,"unit"=>$val_5,"amount"=>$val_8));
$rest=R::exec("UPDATE stockitems SET quantity=quantity-'$val_4' WHERE id='$val_1'");

	// $res=parent::insert("grn_main",array());
if($res>=0){
		$rr=R::getAll("SELECT * FROM issuenotesub WHERE id=$res");	
		foreach ($rr as $val) {
...
function deleteissue(){
 $data=null;
    	 $id=$_POST['id'];
	     R::exec("DELETE FROM issuenotesub WHERE isn_no='$id'");
