...
<?php
class Invoice_returnedit_Model extends model
{
	...
	function load(){
		...
		$res=R::getAll("SELECT * FROM invoicertnmaster ORDER BY id DESC LIMIT $start,$per_page");	
		if($res){
		foreach ($res as $value) {
		$isnno=$value['invoice_no'];
		echo "  <ul class='data_list' id='ul_".$value['id']."'>
		        <li>".$value['id']."</li>
				<li>".$value['substore']."</li>
				<li>".$value['date']."</li>
				<li></li>
				<li><button type='button' class='btn btn-danger btn_delete' id='d_".$value['id']."'>Delete</button></li>
				</ul>";
		echo "<button type='button' class='btn btn-primary btn_edit2' id='e_".$value['id']."'>Show / Edit Invoice</button>
		      <div id='viewmoreTable'>";
		$rtn_id=$value['id'];
		$result=R::getAll("SELECT * FROM invoicertnsub WHERE rtnno=$rtn_id");
		echo " 	<div class='panel panel-default'>
  				<div><button type='button' class='btn btn-warning add_new' id='".$value['id']."' data-id-isnno='".$value['invoice_no']."'>Add New</button></div>
	        	...
	        	<tbody class='body_".$rtn_id."'>
			 ";
		foreach($result as $val){	
		echo "<tr class='datatr_".$val['id']."'>
				<input type='hidden' value='".$val['item_code']."' class='sdata_val12_".$val['id']."'>
				<td><input type='text' value='".$val['item_code']."' class='sdata_val1_".$val['id']." form-control' style='width:80px' readonly></td>
				<td>
	        	<select id='stockitems' class='sdata_val2_".$val['id']." form-control' data-id-bb='".$val['rtnno']."' name='val_2' required=''>";
	        	echo "<option selected value=";
	        	echo $val['item_code'];
	        	echo ">";
	        	echo $val['item_name'];
	        	echo "</option>";
		$rr=R::getAll("SELECT * FROM invoicertnsub WHERE isn_no='$isnno'");	
		foreach ($rr as $vv) {
		echo "<option value=";
		echo $vv['item_code'];
		echo  ">";
	    ...
	    echo"   </select>
				...
				<td>
					<input type='number' value='".$val['unit_price']."' style='width:100px' class='unitprice data_".$val['id']." sdata_val3_".$val['id']." form-control' data-id-aa='".$val['id']."' data-id-bb='".$val['rtnno']."' numberonly required='' min='1'>
				</td>
				<td>
					<input type='hidden' value='".$val['quantity']."' class='sdata_val11_".$val['id']."'>
				    <input type='number' value='".$val['quantity']."' style='width:100px' class='quantity sdata_val4_".$val['id']." form-control' id='".$val['id']."' data-id-bb='".$val['rtnno']."' numberonly required='' min='1'>
				</td>
	...
	echo"	<td><input type='text' value='".number_format($val['amount'], 2, '.', '')."' id='amount_".$val['id']."' 
			class='totamount_".$val['rtnno']." sdata_val8_".$val['id']." form-control'></td>
				<td><button type='button' class='btn btn-success btn_subedit' id='e_".$val['id']."' data-id-bb='".$val['rtnno']."'>Update</button></td>
				<td><button type='button' class='btn btn-danger btn_subdelete' id='d_".$val['id']."' data-id-bb='".$val['rtnno']."'>Delete</button><td>
				</tr>";
		}
		echo "
		<tr><td></td> <td></td> <td></td> <td></td>
		<td>Total</td><td><span class='sub_total_".$val['rtnno']."'></span></td>
		<td></td> <td></td>
		</tr>
		...
		$query_pag_num=R::getAll("SELECT COUNT(*) FROM substorreturnmaster");	
		if($query_pag_num){
...
function delete_sub(){
 $data=null;
 $id=$_POST['id'];
 $res=parent::delete2("invoicertnsub",$id);
 $data=array("reply"=>"10");
...
$val_16=$_POST['val_14'];
  ...
  $rest=R::exec("UPDATE invoicertnsub SET item_code='$val_3',item_name='$val_4',unit_price='$val_5',quantity='$val_6',unit='$val_7',amount='$val_10' WHERE id='$val_1' AND rtnno='$val_2' ");
     if($rest=='1'){
     $data=array("reply"=>"10");
     }else{
     $data=array("reply"=>"20");    
...
function create_new(){
	$invno=$_POST['inv_no'];
		echo "<tr class='".$_POST['count']."'>
	        	...
	        	echo ">";
	        	echo "</option>";
		$rr=R::getAll("SELECT * FROM invoicesub WHERE invoce_no='$invno'");	
		foreach ($rr as $vv) {
		echo "<option value='".$vv['item_code']."'>".$vv['item_name']."</option>";
		}
	    echo"   </select>
				</td>
				<td>
		...
		echo"
				...
				<td><button type='button' class='btn btn-info new_save' id='newsave_".$_POST['count']."'
				 data-id-rtnno='".$_POST['rtnno']."' data-id-invno='$invno'>Save</button></td>
...
$res=parent::insert("invoicertnsub",array("rtnno"=>$rtnno,"item_code"=>$val_1,"item_name"=>$val_2,"unit_price"=>$val_3,"quantity"=>$val_4,"unit"=>$val_5,"amount"=>$val_8));
	// $res=parent::insert("grn_main",array());
if($res>=0){
		$rr=R::getAll("SELECT * FROM invoicertnsub WHERE id=$res");	
		foreach ($rr as $val) {
		echo "<tr class='datatr_".$res."'>
				<input type='hidden' value='".$val['item_code']."' class='sdata_val12_".$val['id']."'>
				<td><input type='text' value='".$val['item_code']."' class='sdata_val1_".$val['id']." form-control' style='width:80px'></td>
				<td>
	        	<select id='stockitems' class='sdata_val2_".$val['id']." form-control' data-id-bb='".$val['rtnno']."' name='val_2' required=''>";
	        	echo "<option selected value=";
	        	echo $val['item_code'];
	    ...
	    echo"   </select>
				...
				<td>
					<input type='hidden' value='".$val['quantity']."' class='sdata_val11_".$val['id']."'>
				    <input type='number' value='".$val['quantity']."' style='width:100px' class='quantity sdata_val4_".$val['id']."' id='".$val['id']." form-control' data-id-bb='".$val['rtnno']."' numberonly required='' min='1'>
				</td>
				<td><input type='text' value='".$val['unit']."' class='sdata_val5_".$val['id']." form-control'  style='width:60px' readonly></td>";
		echo"
				</td>
...
function deletereturn(){
 $data=null;
    	 $id=$_POST['id'];
	     R::exec("DELETE FROM invoicertnsub WHERE rtnno='$id'");
