...
<?php
class Invoice_edit_Model extends model
{
	...
	function load(){
		...
		$res=R::getAll("SELECT * FROM invoicemaster ORDER BY id DESC LIMIT $start,$per_page");	
		if($res){
		foreach ($res as $value) {
		echo "  <ul class='data_list' id='ul_".$value['id']."'>
		        <li>".$value['id']."</li>
				<li>".$value['customer']."</li>
				<li>".$value['date']."</li>
				<li></li>
				<li><button class='btn btn-primary btn_print' data-toggle='modal' data-target='#addAvailableModal 'id='d_".$value['id']."' >print</button></li>	
		...
		$invoice_id=$value['id'];
		$result=R::getAll("SELECT * FROM invoicesub WHERE invoce_no=$invoice_id");
		...
		echo "<tr class='datatr_".$val['id']."'>
				...
				<td>
	        	<select id='stockitems' class='sdata_val2_".$val['id']." form-control' data-id-aa='".$val['id']."' data-id-bb='".$val['invoce_no']."' name='val_2' required=''>";
	...
	echo"	<td><input type='text' value='".number_format($val['amount'], 2, '.', '')."' id='amount_".$val['id']."' 
			class='totamount_".$val['invoce_no']." sdata_val8_".$val['id']." form-control'></td>
				<td><button type='button' class='btn btn-success btn_subedit' id='e_".$val['id']."' data-id-bb='".$val['invoce_no']."'>Update</button></td>
				<td><button type='button' class='btn btn-danger btn_subdelete' id='d_".$val['id']."' data-id-bb='".$val['invoce_no']."'>Delete</button><td>
				</tr>";
		}
		echo "
		<tr><td></td> <td></td> <td></td> <td></td>
		<td>Total</td><td><span class='sub_total_".$val['invoce_no']."'></span></td>
		<td></td> <td></td>
		</tr>
		...
		$query_pag_num=R::getAll("SELECT COUNT(*) FROM invoicemaster");	
		if($query_pag_num){
...
function delete_sub(){
 $data=null;
 $id=$_POST['id'];
 $res=parent::delete2("invoicesub",$id);
 $data=array("reply"=>"10");
...
$val_16=$_POST['val_14'];
  ...
  $rest=R::exec("UPDATE invoicesub SET item_code='$val_3',item_name='$val_4',unit_price='$val_5',quantity='$val_6',unit='$val_7',amount='$val_10' WHERE id='$val_1' AND invoce_no='$val_2' ");
     if($rest=='1'){
     $data=array("reply"=>"10");
     }else{
     $data=array("reply"=>"20");    
...
$res=parent::insert("invoicesub",array("invoce_no"=>$invno,"item_code"=>$val_1,"item_name"=>$val_2,"unit_price"=>$val_3,"quantity"=>$val_4,"unit"=>$val_5,"amount"=>$val_8));
	// $res=parent::insert("grn_main",array());
if($res>=0){
		$rr=R::getAll("SELECT * FROM invoicesub WHERE id=$res");	
		foreach ($rr as $val) {
	    ...
	    echo"   </select>
				...
				<td>
					<input type='number' value='".$val['unit_price']."' style='width:100px' class='unitprice data_".$val['id']." sdata_val3_".$val['id']." form-control' data-id-aa='".$val['id']."' data-id-bb='".$val['invoce_no']."' numberonly required='' min='1'>
				</td>
				<td>
				    <input type='hidden' value='".$val['quantity']."' class='sdata_val11_".$val['id']."'>
				    <input type='number' value='".$val['quantity']."' style='width:100px' class='quantity sdata_val4_".$val['id']."' id='".$val['id']." form-control' data-id-bb='".$val['invoce_no']."' numberonly required='' min='1'>
				</td>
...
function deleteinvoice(){
 $data=null;
    	 $id=$_POST['id'];
	     R::exec("DELETE FROM invoicesub WHERE invoce_no='$id'");
...
function printinvoice(){
	$data=null;
			$id=$_POST['id'];
			// $rest=R::getAll("SELECT * FROM invoicemaster WHERE id='$id' LEFT JOIN invoicesub ON invoicesub.id = invoicesub.invoce_no");
			R::getAll("SELECT * FROM invoicemaster WHERE id='$id'");
			// $sub = $qw[0]['id']

			$rest=R::getAll("SELECT * FROM invoicesub WHERE invoce_no='$id'");
