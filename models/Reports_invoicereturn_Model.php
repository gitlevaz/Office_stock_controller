...
<?php
class Reports_invoicereturn_Model extends model
{
	...
	function load(){
		echo "<div class='col-sm-12 col-md-12'>";
			...
			$res=R::getAll("SELECT * FROM invoicertnsub ORDER BY id DESC LIMIT $start,$per_page");	
			if($res){
			echo "
			...
			$query_pag_num=R::getAll("SELECT COUNT(*) FROM invoicertnsub");	
			if($query_pag_num){
	...
	function search(){
		...
		$to_date=$_POST['val2'];
	    $rest=R::exec("DELETE from invoicertnreport");
		$query=R::getAll("SELECT id FROM invoicertnmaster WHERE date BETWEEN '$from_date' AND '$to_date'");
        foreach ($query as $value) {
        	$ids=$value['id'];
        	$res=R::getAll("SELECT * FROM invoicertnsub WHERE rtnno='$ids'");
	        	foreach ($res as $xx) {
						$res=parent::insert("invoicertnreport",array("rtnno"=>$xx['rtnno'],"item_code"=>$xx['item_code'],"item_name"=>$xx['item_name'],"unit_price"=>$xx['unit_price'],"quantity"=>$xx['quantity'],"unit"=>$xx['unit'],"amount"=>$xx['amount']));
		...
		echo "<div class='col-sm-12 col-md-12'>";
			...
			$aa=$page+1;
			echo "<input type='hidden' value='$aa' class='hidden_val2' name='hidden_val2'>";
			$res=R::getAll("SELECT * FROM invoicertnreport ORDER BY id DESC LIMIT $start,$per_page");	
			if($res){
			echo "
			    <div class='table-responsive cage1' id=''>
			    <table class='table table-condensed default_table'>
			...
			$query_pag_num=R::getAll("SELECT COUNT(*) FROM invoicertnreport");	
			if($query_pag_num){
			foreach ($query_pag_num as $key => $value) {
				$count= $value['COUNT(*)'];
			}	
