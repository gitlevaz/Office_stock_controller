...
<?php
class Itemcategory_edit_Model extends model
{
...
$res=R::getAll("SELECT * FROM itemcategory ORDER BY id DESC LIMIT $start,$per_page");	
if($res){
echo "
...
echo "<tr><td>".$value['category_name']."</td><td><button type='button' class='btn btn-primary btn_edit' id='e_".$value['id']."'>Edit</button></td><td><button type='button' class='btn btn-danger btn_delete' id='d_".$value['id']."'>Delete</button><td></tr>";
}
...
$query_pag_num=R::getAll("SELECT COUNT(*) FROM itemcategory");	
if($query_pag_num){
...
$id=$va[1];
$result=R::getAll("SELECT * FROM itemcategory WHERE id=$id");
if($result){
foreach ($result as $value) {
$data=array("reply"=>"10","val_1"=>$value['category_name'],"val_10"=>$value['id']);
}
}else{
$data=array("reply"=>"20");    
}
...
$id=$_POST['hidden_val1'];
     $rest=R::exec("UPDATE itemcategory SET category_name='$val_1' WHERE id='$id'");
     if($rest=='1'){
     $data=array("reply"=>"10");
     }else{
     $data=array("reply"=>"20");    
...
$id=$va[1];
$res=parent::delete2("itemcategory",$id);
$data=array("reply"=>"10");
echo json_encode($data);   
}
