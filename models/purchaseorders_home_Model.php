...
<?php
class purchaseorders_home_Model extends model
{
	...
	function save_main(){
		...
		$res=parent::insert("ordermaster",array("date"=>$_POST['val_3'],"act_date"=>$date));
	...
	$ex=explode(",",$vl);
		...
		$res=parent::insert("ordersub",array("order_no"=>$xx[1],"item_code"=>$xx[0],"item_name"=>$itme_name,"unit_price"=>$xx[3],"quantity"=>$xx[4],"unit"=>$xx[5],"amount"=>$xx[6],"status"=>'0'));
	// $res=parent::insert("grn_main",array());
	     if($res>=0){
	     	$data=array("reply"=>"10");
	     }else{
	     $res=parent::delete2("ordermaster",$invoice_id);
	     R::exec( 'DELETE ordersub where order_no=$grn_id');
	     $data=array("reply"=>"20");
	     echo json_encode($data);
	     return false;	
	     }
    ...
    function remove_invoice(){
    	$id=$_POST['id'];
	     ...
	     R::exec( 'DELETE ordersub where order_no=$id ');
    }
