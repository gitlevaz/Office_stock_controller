...
<?php
class Reports_invoicertnsummery_Model extends model
{
	...
	function load(){
		echo "<div class='col-sm-12 col-md-12'>";
			...
			$totalvalue=0;
			$indexno=$value['id'];
			$rr=R::getAll("SELECT SUM(amount) as aa FROM invoicertnsub WHERE rtnno='$indexno'");
            foreach ($rr as $vv) {
             $totalvalue=$vv['aa'];
            }
            $total+=$totalvalue;
			...
			$query_pag_num=R::getAll("SELECT COUNT(*) FROM invoicertnmaster");	
			if($query_pag_num){
	...
	function search(){
		...
		echo "<div class='col-sm-12 col-md-12'>";
			...
			$aa=$page+1;
			echo "<input type='hidden' value='$aa' class='hidden_val2' name='hidden_val2'>";
			$res=R::getAll("SELECT * FROM invoicertnmaster WHERE date BETWEEN '$from_date' AND '$to_date' ORDER BY id DESC LIMIT $start,$per_page");	
			if($res){
			echo "
			    <div class='table-responsive cage1' id=''>
			    <table class='table table-condensed default_table'>
