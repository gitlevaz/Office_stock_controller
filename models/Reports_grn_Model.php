...
<?php
class Reports_grn_Model extends model
{
	...
	function load(){
		echo "<div class='col-sm-12 col-md-12'>";
			...
			$res=R::getAll("SELECT * FROM grnsub ORDER BY id DESC LIMIT $start,$per_page");	
			if($res){
			echo "
			...
			$query_pag_num=R::getAll("SELECT COUNT(*) FROM grnsub");	
			if($query_pag_num){
	...
	function search(){
		...
		$to_date=$_POST['val2'];
	    $rest=R::exec("DELETE from grnreport");
		$query=R::getAll("SELECT id FROM grnmain WHERE date BETWEEN '$from_date' AND '$to_date'");
        foreach ($query as $value) {
        	$grnno=$value['id'];
        	$res=R::getAll("SELECT * FROM grnsub WHERE grn_no='$grnno'");
	        	foreach ($res as $xx) {
		            $res=parent::insert("grnreport",array("grn_no"=>$xx['grn_no'],"item_code"=>$xx['item_code'],"item_name"=>$xx['item_name'],"unit_price"=>$xx['unit_price'],"quantity"=>$xx['quantity'],"unit"=>$xx['unit'],"containers"=>$xx['containers'],"container"=>$xx['container'],"amount"=>$xx['amount'],"exp_date"=>$xx['exp_date'],"foc"=>$xx['foc']));
		...
		echo "<div class='col-sm-12 col-md-12'>";
			...
			$aa=$page+1;
			echo "<input type='hidden' value='$aa' class='hidden_val2' name='hidden_val2'>";
			$res=R::getAll("SELECT * FROM grnreport ORDER BY id DESC LIMIT $start,$per_page");	
			if($res){
			echo "
			    <div class='table-responsive cage1' id='grncontent'>
			    <table class='table table-condensed default_table'>
			...
			$query_pag_num=R::getAll("SELECT COUNT(*) FROM grnreport");	
			if($query_pag_num){
			foreach ($query_pag_num as $key => $value) {
				$count= $value['COUNT(*)'];
			}	
