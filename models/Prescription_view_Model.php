...
<?php
class Prescription_view_Model extends model
{
    ...
    function load_dataauto(){
		...
		if($res){
			foreach ($res as $val) {
                echo "<a href='#demo".$val['id']."' data-toggle='collapse' class='view_link' id='row_".$val['id']."'>";
                ...
                echo " Time : <span class='label label-default'>".$val['act_time']."</span>";
                echo "</a>";
                echo "<p class='doc_note'><strong>Note : </strong>".$val['note']."</p>";
				}
				...
				if($re){
				foreach ($re as $vv) {
			 	echo "<td><button type='button' name='prval_".$va['id']."' class='btn btn-warning proceed_btn prval_".$va['id']."'>Proceed</button></td>
    ...
    function load_prescriptions(){
		...
		if($res){
			foreach ($res as $val) {
                echo "<a href='#demo".$val['id']."' data-toggle='collapse' class='view_link'>";
                echo "<span class=''>".$val['id']."</span>";
				$cus_id=$val['patient'];
				$rest=R::getAll("SELECT * FROM customers WHERE id='$cus_id'");	
				foreach ($rest as $value) {
				...
				if($re){
				foreach ($re as $vv) {
			 	echo "<td><button type='button' name='prval_".$va['id']."' class='btn btn-warning proceed_btn prval_".$va['id']."'  disabled>Proceed</button></td>
    ...
    function update_status(){
    	...
    	$rx=R::getAll("SELECT id FROM prescriptionsub WHERE id='$pres_id'");
    	if($rx){
