...
<?php
class Substore_returnsedit_Model extends model
{
	...
	function load(){
		...
		$res=R::getAll("SELECT * FROM substorreturnmaster ORDER BY id DESC LIMIT $start,$per_page");	
		if($res){
		...
		echo "<button type='button' class='btn btn-primary btn_edit2' id='e_".$value['id']."'>Edit</button>
		      <div id='viewmoreTable'>";
		$rtn_id=$value['id'];
		$result=R::getAll("SELECT * FROM substorreturnsub WHERE rtnno=$rtn_id");
		echo " 	<div class='panel panel-default'>
  				<div><button type='button' class='btn btn-warning add_new' id='".$value['id']."' data-id-isnno='".$value['isn_no']."'>Add New</button></div>
		...
		echo "<tr class='datatr_".$val['id']."'>
	        	...
	        	echo $val['item_name'];
	        	echo "</option>";
		$rr=R::getAll("SELECT * FROM issuenotesub WHERE isn_no='$isnno'");	
		foreach ($rr as $vv) {
		echo "<option value=";
		echo $vv['item_code'];
		echo  ">";
...
function delete_sub(){
 $data=null;
 $id=$_POST['id'];
 $res=parent::delete2("substorreturnsub",$id);
 $data=array("reply"=>"10");
...
$val_10=$_POST['val_10'];
  ...
  $rest=R::exec("UPDATE substorreturnsub SET item_code='$val_3',item_name='$val_4',unit_price='$val_5',quantity='$val_6',unit='$val_7',amount='$val_10' WHERE id='$val_1' AND rtnno='$val_2' ");
     if($rest=='1'){
     $data=array("reply"=>"10");
     }else{
     $data=array("reply"=>"20");    
...
function create_new(){
	$isnno=$_POST['isn_no'];
		...
		echo"
				...
				<td><button type='button' class='btn btn-info new_save' id='newsave_".$_POST['count']."' data-id-rtnno='".$_POST['rtnno']."'>Save</button></td>
...
$res=parent::insert("substorreturnsub",array("rtnno"=>$rtnno,"item_code"=>$val_1,"item_name"=>$val_2,"unit_price"=>$val_3,"quantity"=>$val_4,"unit"=>$val_5,"amount"=>$val_8));
	// $res=parent::insert("grn_main",array());
if($res>=0){
		$rr=R::getAll("SELECT * FROM substorreturnsub WHERE id=$res");	
		foreach ($rr as $val) {
...
function deletereturn(){
 $data=null;
    	 $id=$_POST['id'];
	     R::exec("DELETE FROM substorreturnsub WHERE rtnno='$id'");
