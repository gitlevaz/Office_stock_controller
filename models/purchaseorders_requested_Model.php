<?php
class purchaseorders_requested_Model extends model
{
	...
	function load(){
		...
		$res=R::getAll("SELECT * FROM ordermaster WHERE status='0' ORDER BY id DESC LIMIT $start,$per_page");	
		if($res){
		foreach ($res as $value) {

// 				$date=date_create($value['date']);
// date_add($date,date_interval_create_from_date_string("20.5 hours"));
// echo date_format($date,"Y-m-d H:i:s");
		...
		echo "	<li><button type='button' class='btn btn-success btn_confirm' id='d_".$value['id']."'>Confirm Order</button></li>";
		}
	...
	echo"	<td><input type='text' value='".number_format($val['amount'], 2, '.', '')."' id='amount_".$val['id']."' 
		...
		$query_pag_num=R::getAll("SELECT COUNT(*) FROM ordermaster WHERE status='0'");	
		if($query_pag_num){
...
function confirmorder(){
	$id=$_POST['id'];
    $rest=R::exec("UPDATE ordersub SET status='1' WHERE order_no='$id' ");
    $res=R::exec("UPDATE ordermaster SET status='1' WHERE id='$id' ");
     if($res=='1'){
       	 $data=array("reply"=>"10");
	    }else{
	     $data=array("reply"=>"20");    
